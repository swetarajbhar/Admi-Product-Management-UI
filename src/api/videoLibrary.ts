...
export const videoLibraryList = async (
  payload: IVideoLibraryPayload
): Promise<TStandardObject> => {
  try {
    const response = await getAxiosInstance().post(
      API_URLS.V1.VIDEO_LIBRARY_LIST,
      payload
    );
    return {
...
export const videoLibraryGetScreenshots = async (
  fileURL: string
): Promise<any> => {
  try {
    const response = await getAxiosInstance().post(
      API_URLS.V1.VIDEO_LIBRARY_VIDEO_SCREENSHOTS,
      { fileURL }
    );
    const images: any = [[]];
...
export const videoLibraryDeleteSection = async (
  payload: IVideoSectionDeletePayload
): Promise<any> => {
  try {
    const response = await getAxiosInstance().post(
      API_URLS.V1.VIDEO_LIBRARY_VIDEO_EDIT,
      payload
    );
...
export const saveVideoData = async (
  payload: ISaveVideoDataPayload
): Promise<any> => {
  try {
    const response = await getAxiosInstance().post(
      API_URLS.V1.SAVE_VIDEO_DATA,
      payload
    );
...
export const deleteVideoRecord = async (
  ...
  try {
    const response = await getAxiosInstance().delete(
      `${API_URLS.V1.DELETE_VIDEO_DATA}/${contentID}`
    );
    return { status: response.status, error: "", data: response.data };
  } catch (error: any) {
    console.error("error", error);
...
export const cloneVideoRecord = async (
  contentID: string | number
): Promise<TAPIResponse> => {
  try {
    const response = await getAxiosInstance().get(
      `${API_URLS.V1.CLONE_VIDEO_RECORD}/${contentID}`
    );
